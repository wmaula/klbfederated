...
server_node <- function(con) {
  function(input, output, session) {
    ...
    investigasi_aktif <- shiny::reactive({
      shiny::req(rv$inv_id)
      # Ikut menyegar setiap kali basis data berubah, misalnya setelah proyek
      # KoboToolbox ditautkan atau status investigasi diubah.
      rv$muat_ulang
    ...
    output$kobo_status <- shiny::renderUI({
      ...
      ada_token <- !is.na(token) && nzchar(token)
      ada_inv <- !is.null(inv)
      ada_taut <- ada_inv && !is.na(inv$kobo_uid)

      langkah <- function(selesai, teks) {
        shiny::tags$li(class = "klb-kecil",
                       shiny::span(class = paste("klb-pill", if (selesai) "ok" else "warn"),
                                   if (selesai) "selesai" else "belum"), " ", teks)
      }
      shiny::tagList(
        shiny::tags$ol(
          langkah(ada_token, "Token KoboToolbox diatur pada menu Pengaturan"),
          langkah(ada_inv, "Investigasi dipilih pada tab Investigasi"),
          langkah(!is.null(rv$aset_kobo), "Daftar proyek dimuat"),
          langkah(ada_taut, if (ada_taut)
                    sprintf("Proyek tertaut: %s (%s)", inv$kobo_nama, inv$kobo_uid)
                  else "Proyek dipilih pada tabel lalu ditautkan"),
          langkah(ada_inv && DBI::dbGetQuery(con,
                    "SELECT COUNT(*) AS n FROM submission WHERE investigasi_id = ?",
                    list(rv$inv_id))$n > 0, "Data ditarik ke node ini")
        )
      )
    })
    ...
    shiny::observeEvent(input$kobo_tautkan, {
      if (is.null(rv$inv_id)) {
        return(notifikasi("Pilih dahulu satu investigasi pada tab Investigasi.", "error"))
      }
      if (is.null(rv$aset_kobo)) {
        return(notifikasi("Klik Muat daftar proyek terlebih dahulu.", "error"))
      }
      baris <- input$tabel_kobo_rows_selected
      if (length(baris) != 1) {
        return(notifikasi("Pilih satu baris proyek pada tabel di bawah, lalu klik Tautkan lagi.", "error"))
      }
    ...
    shiny::observeEvent(input$kobo_impor, {
      ...
      inv <- investigasi_aktif()
      if (is.na(inv$kobo_uid)) {
        return(notifikasi(paste(
          "Investigasi ini belum ditautkan ke proyek KoboToolbox. Klik Muat daftar proyek,",
          "pilih satu baris pada tabel, lalu klik Tautkan proyek terpilih sebelum menarik data."),
          "error"))
      }
      skema <- skema_form()
